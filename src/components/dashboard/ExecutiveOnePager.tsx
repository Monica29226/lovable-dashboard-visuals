...
import { buildYearProjection, computeFiscal, formatMoney, formatPct, topExpenses, type FiscalResult, type TaxBracket, type YearProjection } from "@/lib/executiveProjection";
...
export const ExecutiveOnePager = ({ companyId, companyName, isConnected, year }: Props) => {
  ...
  if (!enhancedView) {
    return <LegacyView companyName={companyName} fiscalYear={fiscalYear} projection={projection} fiscal={fiscal} expenses={expenses} money={money} controls={controls} accordion={accordion} />;
  }
...
const LegacyView = ({ companyName, fiscalYear, projection, fiscal, expenses, money, controls, accordion }: { companyName: string; fiscalYear: number; projection: YearProjection; fiscal: FiscalResult; expenses: ReturnType<typeof topExpenses>; money: (value: number | null | undefined) => string; controls: React.ReactNode; accordion: React.ReactNode }) => {
  ...
  return <section className="dashboard-sans bg-paper"><header className="bg-ink px-6 py-8 md:px-10 md:py-10"><p className="text-[0.7rem] font-semibold uppercase tracking-[0.14em] text-gold">{companyName}</p><h1 className="mt-2 text-2xl font-semibold leading-tight text-primary-foreground md:text-[2rem]">Su situación financiera y fiscal {fiscalYear}</h1><div className="mt-3 h-[5px] w-24 border-y border-gold" /><p className="mt-3 text-sm text-primary-foreground/70">Información real acumulada a {projection.lastClosedLabel} de {fiscalYear} · Montos expresados en colones</p></header>
